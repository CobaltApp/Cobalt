...
sed -i '' 's/"patches": "patch -p1 < src/scripts\/react-native-tor.patch; patch -p1 < scripts\/rn-ldk.patch",/"patches":/g' ./package.json
rm -fr node_modules
echo "Re-installing node_modules"
npm i
echo "Deleting torrific.js and lightning-ldk-wallet.ts content"
cp src/scripts/maccatalystpatches/torrific.js blue_modules/torrific.js
cp src/scripts/maccatalystpatches/lightning-ldk-wallet.ts class/wallets/lightning-ldk-wallet.ts
